...
Markdown 文件已包含：
- 帖子完整内容
- 附件链接（相对路径，如 `asset/03-01/82811515254842180_01_xxx.pdf`）
- 帖子元数据（时间、ID、类型等）
...
**方法 2：使用 download-pdf/download-docx 命令**（如果需要）

如果 Markdown 中包含 file_id 信息，也可以使用：
...
但这会重新下载文件，不推荐。优先使用方法 1 直接读取本地文件。
...
结合 Markdown 文件内容和文档附件文本，按报告格式输出，将输出后的报告保存到report/MM-DD.md文件。
